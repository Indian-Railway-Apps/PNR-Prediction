...
public class SaveRegIdCommand extends Command {
	...
	protected void execute(ResultObject result) throws Exception {
		...
		try{
			...
			Log.v(PPApplication.TAG, "Saving GCM RegId with our Server");
			...
			if(line.toString().contentEquals("Success")){
				...
				Log.v(PPApplication.TAG, "GCM RegId saved successfully on our server");
			}
			
		}
		catch (Exception e){
			// Nothing to do
			Log.w(PPApplication.TAG, "Following error occured while saving GCM RegId with our servers:");
